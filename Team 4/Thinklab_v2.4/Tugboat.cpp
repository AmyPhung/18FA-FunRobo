...
void Tugboat::processData() { // Takes irDataArray, pixyDataArray as input. Modifies irDecisionArray, pixyDecisionArray
  for (int i=0; i<NUM_PTS; i++) {
    pixyDecisionArray[i] = map(pixyDataArray[i],-100, 100, 5, 150);
    irDecisionArray[i] = map(irDataArray[i],-100, 100, 5, 30); // 5-30 is range of sensors, -100 to 100 is speed
    dataDecisionArray[i] = (pixyDecisionArray[i] + irDecisionArray[i])/2;
  }
  for (int i = 0; i < NUM_PTS; i++){
    Serial.print(i);
    Serial.print(": ");
    Serial.print(dataDecisionArray[i]);
    Serial.print("\t");
  }
...
void Tugboat::arbiter() { // Takes irDecisionArray, pixyDecisionArray, state as input. Modifies heading, velocity, and state
  ...
  Array<int> decisionArray = Array <int>(dataDecisionArray,NUM_PTS); //TODO: Make this take sum of pixyDecisionArray and irDecisionArray
  ...
  velocity = max_val; // TODO: Parameterize this and make speed between -100 and 100
